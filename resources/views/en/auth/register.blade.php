...
<div class="container d-flex justify-content-center align-items-center min-vh-100">
    <div class="card shadow-lg p-5" style="max-width: 600px; width: 100%; border-radius: 15px; position: relative; overflow: hidden;">
        ...
        <form method="POST" action="{{ route('register') }}" style="z-index:1; position:relative;">
            ...
            <!-- Nazionalità -->
            <div class="mb-4">
                <label for="nazionalita" class="form-label">Nationality</label>
            ...
            <!-- Codice Fiscale -->
            <div class="mb-4" id="codiceFiscaleGroup" style="display: none;">
                <label for="codice_fiscale" class="form-label">Fiscal Code</label>
                <input id="codice_fiscale" type="text" maxlength="16" class="form-control @error('codice_fiscale') is-invalid @enderror" name="codice_fiscale" value="{{ old('codice_fiscale') }}" placeholder="Only Italian">
                @error('codice_fiscale') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            <!-- Sesso -->
            <div class="mb-4">
                <label for="sesso" class="form-label">Sex</label>
                <select id="sesso" name="sesso" class="form-select @error('sesso') is-invalid @enderror" required>
                    <option value="" disabled {{ old('sesso') ? '' : 'selected' }}>Select</option>
                    <option value="M" {{ old('sesso') == 'M' ? 'selected' : '' }}>Male</option>
                    <option value="F" {{ old('sesso') == 'F' ? 'selected' : '' }}>Female</option>
                    <option value="Altro" {{ old('sesso') == 'Altro' ? 'selected' : '' }}>Other</option>
                </select>
            ...
            <div class="row mb-4">
                ...
                <div class="col">
                    <label for="luogo_nascita" class="form-label">Place of Birth</label>
            ...
            <div class="mb-4">
                <label for="indirizzo_residenza" class="form-label">Residence address</label>
...
<script>
    ...
    $(document).ready(function() {
        $('#nazionalita').select2({
            ...
            // Mostra/nascondi campo codice fiscale
            function toggleFiscalCode() {
                const selected = $('#nazionalita').val()?.toLowerCase();
                const cfGroup = $('#codiceFiscaleGroup');
                if (selected === 'italia' || selected === 'italy') {
                    cfGroup.slideDown(200);
                } else {
                    cfGroup.slideUp(200);
                    $('#codice_fiscale').val(''); // resetta se non serve
                }
            }

            // Al cambio nazionalità
            $('#nazionalita').on('change', toggleFiscalCode);

            // Se già selezionata (in caso di validazione fallita)
            toggleFiscalCode();
