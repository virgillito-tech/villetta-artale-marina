                    ...
                    <div class="card shadow-sm border-0 rounded-4 h-100">
                        <div class="card-body d-flex flex-column">
                            <h5 class="card-title">
                                ...
                                From {{ \Carbon\Carbon::parse($prenotazione->data_inizio)->format('d/m/Y') }}
                                to {{ \Carbon\Carbon::parse($prenotazione->data_fine)->format('d/m/Y') }}
                            </h5>
                            
                            <ul class="list-unstyled text-body-secondary mb-3">
                                <li><i class="fas fa-home fa-fw me-2"></i>Reservation: {{ $prenotazione->codice_prenotazione }}</li>
                                <li><i class="fas fa-users fa-fw me-2"></i> People: {{ $prenotazione->numero_persone }}</li>
                                <li><i class="fas fa-door-open fa-fw me-2"></i> Rooms: {{ $prenotazione->numero_stanze }}</li>
                               <li><i class="fas fa-euro-sign fa-fw me-2"></i> Total Price: €{{ number_format($prenotazione->prezzo_totale, 2, ',', '.') }}</li>
                               <li><i class="fas fa-credit-card fa-fw me-2"></i> Payment Method: {{ $prenotazione->tipo_pagamento }}</li>
                            </ul>
                            ...
                            <div class="mt-auto">
                                <p class="mb-2"><i class="fas fa-info-circle fa-fw me-2"></i> State: 
                                    <span class="badge 
                                    ...
                                    @if($giorniMancanti > 30)
                                        <form action="{{ route('prenotazioni.annulla', $prenotazione->id) }}" method="POST" 
                                            ...
                                            <button type="submit" class="btn btn-outline-danger btn-sm mt-2">
                                                <i class="fas fa-times me-2"></i> Cancel and Request a Refund
                                            </button>
                                        </form>
                                    @elseif($giorniMancanti > 0)
                                        <p class="text-muted small mt-2">You cannot cancel online (less than 30 days before arrival).</p>
                                    @else
                                        <p class="text-muted small mt-2">Booking completed.</p>
                                    @endif

                                @elseif($prenotazione->stato === 'annullata')
                                    <p class="text-danger small mt-2">Reservation cancelled.</p>
                                @endif
                            </div>
